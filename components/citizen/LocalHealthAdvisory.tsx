...
export const LocalHealthAdvisory: React.FC = () => {
    ...
    if (status === 'loading') {
        return (
            <Card className="p-20 border-teal-100 flex flex-col items-center justify-center space-y-6 text-center rounded-[3rem] bg-white shadow-xl">
                <div className="relative">
                    <Loader2 className="h-16 w-16 text-teal-600 animate-spin" />
                    <MapPin className="h-6 w-6 text-teal-600 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" />
                </div>
                <div className="space-y-2">
                    <p className="text-2xl font-black text-zinc-900">Locating... </p>
    ...
    if (status === 'success' && location && aqiData) {
        ...
        return (
            <Card className="p-1 border-teal-100 shadow-2xl overflow-hidden rounded-[3.5rem] bg-white group animate-in fade-in zoom-in-95 duration-700">
                <div className="p-8 md:p-14 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
                    <div className="lg:col-span-5 space-y-8">
                        <div className="flex items-center gap-3">
                            <Badge className="bg-teal-600 text-white border-none px-4 py-1.5 rounded-full font-bold flex items-center gap-2">
                                <MapPin className="h-4 w-4" /> {location.name}
                            </Badge>
                            <Badge variant="outline" className="text-[10px] uppercase font-black tracking-[0.2em] text-zinc-400 border-zinc-200">Local Resource</Badge>
                        </div>

                        <h2 className="text-5xl md:text-6xl font-black text-zinc-900 tracking-tighter leading-[0.9]">
                            Healthy Living <br /> in <span className="text-teal-600 underline decoration-teal-100 underline-offset-8">{location.name}</span>
                        </h2>

                        <div className="flex items-center gap-8 p-8 bg-zinc-50 rounded-[2.5rem] border border-zinc-100 group-hover:border-teal-200 transition-all duration-500 shadow-inner">
                            <div className="space-y-1">
                                <p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">Local AQI Now</p>
                            ...
                            <div className="flex-1">
                                ...
                                <p className="text-sm text-zinc-500 font-bold leading-tight">{display.description}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="lg:col-span-7 space-y-10">
                        <div className="space-y-6">
                            <h4 className="font-black text-2xl text-zinc-900 flex items-center gap-3">
                                <div className="h-10 w-10 rounded-xl bg-teal-600 text-white flex items-center justify-center">
                                    <ShieldAlert className="h-6 w-6" />
                                </div>
                                Real-time City Advisory
                            </h4>
                            <div className="p-8 bg-teal-50/50 rounded-[2rem] border-l-8 border-teal-500 shadow-sm">
                                <p className="text-zinc-800 text-xl font-bold leading-relaxed italic animate-in fade-in slide-in-from-left-4 duration-700">
                                    "{display.message}"
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div className="p-6 bg-white border border-zinc-100 rounded-3xl shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                                <div className="flex items-center gap-4 mb-4">
                                    <div className="h-10 w-10 rounded-xl bg-orange-100 text-orange-600 flex items-center justify-center">
                                        <Wind className="h-5 w-5" />
                                    </div>
                                    <p className="font-black text-zinc-900 uppercase text-xs tracking-widest">Primary concern</p>
                                </div>
                            ...
                            <div className="p-6 bg-white border border-zinc-100 rounded-3xl shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                                <div className="flex items-center gap-4 mb-4">
                                    ...
                                    <p className="font-black text-zinc-900 uppercase text-xs tracking-widest">Recommended Action</p>
                                </div>
                                <p className="text-sm text-zinc-500 font-medium">{display.action}</p>
                            </div>
                        </div>
                        ...
                        {(location.name.toLowerCase().includes('mumbai') || location.name.toLowerCase().includes('delhi')) && (
                            <div className="p-6 bg-zinc-900 rounded-[2rem] text-white flex items-center justify-between">
                                <div className="space-y-1">
    ...
    return (
        <Card className="p-12 border-red-100 bg-red-50 text-center space-y-6 rounded-[3rem]">
            <div className="h-16 w-16 bg-red-200 text-red-700 rounded-full flex items-center justify-center mx-auto">
                <ShieldAlert className="h-8 w-8" />
            </div>
            <div className="space-y-2">
                <h3 className="text-2xl font-black text-red-900">Location Access Required</h3>
                <p className="text-red-600/80 font-medium max-w-sm mx-auto">We need your permission to provide local health intelligence. Please enable location access in your browser.</p>
            </div>
            <button
                onClick={handleEnableLocation}
                className="bg-red-600 text-white font-black px-10 py-4 rounded-2xl hover:bg-red-700 transition-all shadow-lg active:scale-95"
            >
