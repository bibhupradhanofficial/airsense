...
export const AQIScaleExplainer: React.FC = () => {
    return (
        <Card className="p-8 border-zinc-100 shadow-sm overflow-hidden">
            <h3 className="text-2xl font-black text-zinc-900 mb-8 tracking-tight">Understanding the AQI Scale</h3>
            <Accordion type="single" collapsible className="w-full space-y-4">
                {SCALE_DATA.map((item, i) => (
                    <AccordionItem key={i} value={`item-${i}`} className="border rounded-2xl border-zinc-100 px-6 overflow-hidden">
                        <AccordionTrigger className="hover:no-underline py-6">
                            <div className="flex items-center gap-6 w-full pr-4">
                                <div className="h-12 w-24 rounded-xl flex items-center justify-center text-white font-black text-sm shadow-sm" style={{ backgroundColor: item.color }}>
                                    {item.range}
                                </div>
                                <div className="text-left flex-1">
                                    <p className="font-black text-lg" style={{ color: item.color }}>{item.category}</p>
                                    <p className="text-xs text-zinc-400 font-bold uppercase tracking-widest mt-1">Impact Group: {item.who}</p>
                                </div>
                            </div>
                        </AccordionTrigger>
                        <AccordionContent className="pb-6 pt-2 border-t border-zinc-50">
                            <div className="space-y-4">
                                <p className="text-zinc-600 font-medium leading-relaxed">
                                    <span className="text-zinc-900 font-black uppercase text-[10px] tracking-widest block mb-1">Recommended Action</span>
                                    {item.action}
                                </p>
                            </div>
                        </AccordionContent>
