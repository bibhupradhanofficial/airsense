...
export const PollutantEncyclopedia: React.FC = () => {
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {POLLUTANTS.map((p) => (
                <Card key={p.name} className="p-6 border-zinc-100 shadow-sm hover:shadow-md transition-all group overflow-hidden relative">
                    <div className="relative z-10 space-y-6">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                <div className="h-12 w-12 rounded-2xl bg-teal-50 text-teal-600 flex items-center justify-center group-hover:bg-teal-600 group-hover:text-white transition-all duration-500">
                                    {p.icon}
                                </div>
                                <div className="text-left">
                                    <h4 className="font-black text-xl text-zinc-900 tracking-tight">{p.name}</h4>
                                    <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">{p.fullName}</p>
                                </div>
                            </div>
                            <div className="text-right">
                                <Badge variant="outline" className="text-[10px] border-zinc-200">WHO Alert</Badge>
                                <p className="text-xs font-bold text-red-500 mt-1">{p.whoLimit}</p>
                            </div>
                        </div>

                        <p className="text-sm text-zinc-500 line-clamp-2 leading-relaxed text-left">
                            {p.description}
                        </p>

                        <div className="grid grid-cols-2 gap-4 pt-4 border-t border-zinc-50">
                            <div className="space-y-2">
                                <p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">Sources</p>
                                <ul className="space-y-1">
                                    {p.sources.slice(0, 2).map((s, i) => (
                                        <li key={i} className="text-xs text-zinc-600 flex items-center gap-1.5">
                                            <div className="h-1 w-1 rounded-full bg-teal-400" /> {s}
                                        </li>
                                    ))}
                                </ul>
                            </div>
                            <div className="space-y-2">
                                <p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">Health Risks</p>
                                <ul className="space-y-1">
                                    {p.effects.slice(0, 2).map((e, i) => (
                                        <li key={i} className="text-xs text-red-500/80 flex items-center gap-1.5 font-medium">
                    ...
                    <div className="absolute -bottom-8 -right-8 h-24 w-24 bg-zinc-50 rounded-full group-hover:bg-teal-50/50 transition-colors duration-500" />
                </Card>
            ))}
        </div>
    );
